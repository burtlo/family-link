/*
 * h12 — Same hangout PTT as h11, plus an LCD that refreshes while PCM moves.
 * Paint on a 500 ms UI task, never per audio frame. -- PASS h12 if both
 * directions still work (the h11 bar) without a glitch on every paint.
 */
...
static void ui_task(void *arg)
{
    ...
    int last_rx = -1;
    while (1) {
        if (s_tx != last_tx || s_rx != last_rx || s_session) {
            last_tx = s_tx;
            last_rx = s_rx;
            snprintf(line, sizeof(line),
                     "LIVE  floor=%s\ntx=%d  rx=%d",
                     s_floor[0] ? s_floor : "-", s_tx, s_rx);
...
void app_main(void)
{
    ...
    if (esp_websocket_client_start(s_ws) != ESP_OK) {
        demo_fail("h12", "ws start");
        return;
    }

    xTaskCreate(ptt_task, "ptt", 4096, NULL, 5, NULL);
    xTaskCreate(ui_task, "ui", 3072, NULL, 3, NULL);
    ESP_LOGI(TAG, "LCD paints every 500 ms, not per PCM frame. uri=%s", uri);
}
